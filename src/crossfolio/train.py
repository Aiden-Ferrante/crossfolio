...
@torch.no_grad()
def eval_sharpe(model, loader, device) -> float:
    """Deterministic Sharpe of excess returns over every anchor in the loader."""
    model.eval()
    excess = []
    ...
    e = torch.cat(excess)
    return float(e.mean() / (e.std(unbiased=True) + 1e-8))


def train(model_name: str, cfg: Cfg | None = None, panel: Panel | None = None) -> Path:
    cfg = cfg or Cfg()
    ...
    run_dir = RUNS / f"{time.strftime('%Y%m%d-%H%M%S')}-{model_name}-{cfg.loss.name}"
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        if vs > best:
            ...
            checkpoint(epoch, vs)
            print(f"  epoch {epoch:3}: val_sharpe {vs:+.4f} *")
        elif epoch - best_epoch >= cfg.train.patience:
            print(f"  epoch {epoch:3}: val_sharpe {vs:+.4f} — early stop "
